...
**작업 시작 전에 반드시 `wiki/CONTEXT.md`를 먼저 읽어라.**
원본 소스 파일을 직접 읽기 전에 위키 기사를 활용하면 토큰을 대폭 절약할 수 있다.

| 작업 유형 | 읽어야 할 위키 기사 |
|-----------|---------------------|
| 게임 컨셉/방향 | `wiki/topics/game-overview.md` |
| 새 씬/UI 개발 | `wiki/topics/design-system.md` + `wiki/topics/ui-scenes.md` |
| 게임플레이 로직 | `wiki/topics/game-scene.md` |
| 레벨/난이도 | `wiki/topics/level-system.md` |
| 이펙트/사운드 | `wiki/topics/special-fx.md` |
| 광고/수익 | `wiki/topics/monetization.md` |
| 빌드/환경 | `wiki/topics/tech-stack.md` |
| Phaser 패턴 | `wiki/concepts/phaser-patterns.md` |
| UX 방향 | `wiki/concepts/dessert-game-ux.md` |

위키 기사에 `coverage: low`가 표시된 섹션이 있을 때만 원본 파일을 직접 읽는다.
