...
- **100개 구현 레벨**: 난이도 구간, 세부 그룹, 생성 규칙과 개별 조정을 분리
...
현재 제공하는 100개 레벨을 모두 손으로 작성하지 않고 여러 단계의 규칙을 조합해 생성합니다.
...
`?qa=1&qaLevel=1&qaSeed=1492`처럼 실행하면 자동 QA 전용 Bridge가 켜집니다. 일반 사용자 실행에는 노출되지 않습니다.

- Level과 Seed를 지정한 결정적 시작 상태
- DOM JSON 상태 Snapshot
- DOM JSON 명령·응답으로 실제 `GameScene` Swap 실행
- 애니메이션과 Cascade가 끝난 뒤의 상태 반환

이 Bridge는 별도 `game-qa-agent`가 브라우저에서 같은 시작 상태와 한 행동 결과를 재현하는 데 사용합니다.
